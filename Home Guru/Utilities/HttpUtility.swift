...
struct HttpUtility
{
    ...
    func postApiDataWithMultipartForm<T:Decodable>(requestUrl: URL, request: ImageRequest, resultType: T.Type, completionHandler:@escaping(_ result: T)-> Void)
    {
        ...
        urlRequest.addValue("eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOjYsImlhdCI6MTYwODYyOTA0OH0.ktuO-f_lC9-pmcgKqkZPNBozJVpyafJOPATe6Y2BQKg", forHTTPHeaderField: "x-auth-token")
        var requestData = Data()

        requestData.append("--\(boundary)\r\n" .data(using: .utf8)!)
        requestData.append("content-disposition: form-data; name=\"profilePic\" \(lineBreak + lineBreak)" .data(using: .utf8)!)
        requestData.append(request.profilePic .data(using: .utf8)!)

        requestData.append("\(lineBreak)--\(boundary)\r\n" .data(using: .utf8)!)
        requestData.append("content-disposition: form-data; name=\"studentId\" \(lineBreak + lineBreak)" .data(using: .utf8)!)
        requestData.append("\(request.studentId + lineBreak)" .data(using: .utf8)!)
