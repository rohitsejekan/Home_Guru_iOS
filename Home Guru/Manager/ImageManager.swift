...
struct ImageManager
{
    func uploadImage(data: Data, stuId: String,completionHandler: @escaping(_ result: ImageResponse) -> Void)
    {
        let httpUtility = HttpUtility()

        let imageUploadRequest = ImageRequest(profilePic: data.base64EncodedString(), studentId: stuId)
